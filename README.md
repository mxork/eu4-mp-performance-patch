...
1. a customized nakama-cpp client library which loads the other patches
...
The mod only needs to run by the host. It does not change the game's checksum, so its compatible with unmodded players. Non-hosts can see some speedup using the `--fastmp` option, which can be useful on old machines (though it won't fix your internet).
...
And everything should just work! Have fun hosting MP games at a faster and more consistent speed.

Alternatively, you can add the command line flags to your steam launch options:

![Steam launch options](./launchoptions.png)

The Paradox launcher may change how it handles the launch options, so calling the executable directly is more reliable.

## Details

EU4 multiplayer performance is mostly determined by three factors:

1. Network latency and variance. A latency spike lasting a few seconds will force the game speed to decrease.
...
3. The settings `DAYS_BEHIND_LOWER_SPEED` and `DAYS_BEHIND_PAUSE` from defines.lua. These have reasonable defaults, but if you have a fast machine or higher network latency to other players in your game, they essentially guarantee you can't run at higher game speeds.

The DLL patch turns off the daily checksum. An optional steam mod (linked below) adjusts the `DAYS_BEHIND*` settings to be more forgiving. The speed controller smooths out the game speed to avoid triggering changes in game speed or pauses.
...
As much as possible, I've tried to make building everything straight-forward. `libpatcher` has a Docker build file included, mostly as a reference, but also as a check to make sure my machine didn't have any magical state tainting the process.
